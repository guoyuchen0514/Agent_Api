# !/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time : 2023-03-25 10:38
# @Author : 皆人
# @File : assertion_control.py
# @Software: PyCharm
...
from utils.others_tool.models import AssertMethod
from utils.others_tool.exceptions import JsonpathExtractionFailed,SqlNotFound,AssertTypeError
from utils.others_tool.models import load_module_functions
    ...
    def sql_switch_handle(
        ...
        if config.mysql_db.switch:
            ...
            if sql_data != {'sql': None}:
                ...
                if type(res_sql_data) == float or key =='serviceFee':
                    resp_data = '%.2f' % float(resp_data[0])
                    res_sql_data = '%.2f' % res_sql_data
                    self.functions_mapping[name](resp_data, str(res_sql_data), str(message))
                else:
