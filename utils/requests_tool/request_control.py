...
from utils.others_tool.allure_data.allure_tool import allure_attach,allure_step,allure_step_no
...
from utils.others_tool.models import ResponseData, RequestType,TestCase
...
class RequestControl:
    ...
    def multipart_in_headers(
            ...
            request_data:Dict,
            header : dict):
        """判断处理header 为 content-type":multipart/form-data"""
        ...
        if header is None:
            ...
            if 'application/json' in str(header.values()):
                #判断请求参数不为空，并且参数是字典类型
                if request_data and isinstance(request_data,dict):
                    #当Content-Type为 “multipart/form-data”时，需要将数据类型转换成str
    ...
    def _check_params(
        ...
        _data = {
            ...
            "sql_data": self._sql_data_handler(sql_data=ast.literal_eval(str(str(yaml_data.sql))),res=res),
            "yaml_data": yaml_data,
