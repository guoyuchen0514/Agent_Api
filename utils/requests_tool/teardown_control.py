...
from utils.others_tool.jsonpath_date_replace import jsonpath_replace
from utils.mysql_tool.mysql_control import MysqlDB
from utils.logging_tool.log_control import WARNING
from utils.others_tool.models import ResponseData, TearDown, SendRequest, ParamPrepare
from utils.others_tool.exceptions import JsonpathExtractionFailed, ValueNotFoundError
        ...
        if sql_data is not None:
            for i in sql_data:
                if config.mysql_db.switch:
                    ...
                    WARNING.logeer.waring("程序中检查到您数据库开关为关闭状态，已为您跳过删除sql：%s",i)
